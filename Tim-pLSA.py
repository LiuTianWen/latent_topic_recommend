...
class MyLDA:
    ...
    def init_data(self):
        ...
        for z in range(self.topic_num):
            ...
            for t in range(24):
                pr_t_in_z[z][t] = random()
            pr_u_in_z[z] /= pr_u_in_z[z].sum()
            pr_i_in_z[z] /= pr_i_in_z[z].sum()
            pr_t_in_z[z] /= pr_t_in_z[z].sum()
        pz /= pz.sum()

        return pr_u_in_z, pr_i_in_z, pr_t_in_z, pz, pr

    def e_step(self):
        print('E')
        for u in self.pr.keys():
            for i in self.pr[u].keys():
                for t in self.pr[u][i].keys():
                    self.pr[u][i][t] = self.pz * self.pr_u_in_z[:, u] * self.pr_i_in_z[:, i] * self.pr_t_in_z[:, t]
                    self.pr[u][i][t] /= self.pr[u][i][t].sum()
    ...
    def m_step(self):
        ...
        for z in range(len(self.pz)):
            ...
            for t in range(24):
                self.pr_t_in_z[z][t] = 0
            try:
                for u in range(len(self.pr_u_in_z[z])):
                    for check in self.checks.get(u):
                        i = check[0]
                        t = check[2].hour
                        self.pr_u_in_z[z][u] += self.pr[u][i][t][z]
                        self.pr_i_in_z[z][i] += self.pr[u][i][t][z]
                        self.pr_t_in_z[z][t] += self.pr[u][i][t][z]
                        self.pz[z] += self.pr[u][i][t][z]
            except KeyError as e:
                print(u, i, t)
                raise e
            self.pr_u_in_z[z] /= self.pr_u_in_z[z].sum()
            self.pr_i_in_z[z] /= self.pr_i_in_z[z].sum()
            self.pr_t_in_z[z] /= self.pr_t_in_z[z].sum()
        self.pz /= self.pz.sum()
    ...
    def l(self):
        ...
        for u in self.checks.keys():
            for check in self.checks[u]:
                i = check[0]
                t = check[2].hour
                temp = self.pz * self.pr_u_in_z[:, u] * self.pr_i_in_z[:, i] * self.pr_t_in_z[:, t]
                sumv += log(temp.sum())
        return sumv
    ...
    def predict(self, u, i):
        return (self.pz * self.pr_u_in_z[:, u] * self.pr_i_in_z[:, i] * self.pr_t_in_z.T).sum()
